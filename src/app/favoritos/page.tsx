...
export default async function FavoritosPage() {
  ...
  // @ts-ignore
  const userLists: any[] = await FavoriteList.find({
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                  {list.animes.map((anime: any) => (
                    <div
                      key={anime.slug}
                      className="group relative w-full aspect-[2/3] rounded-md overflow-hidden bg-zinc-800 border border-zinc-700"
                    >
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-4">
                        <Link
                          ...
                          className="bg-primary hover:bg-red-700 text-white p-2 rounded-full mb-2"
                        >
                  ...
                  {list.animes.length === 0 && (
                    <div className="col-span-full py-10 text-center text-gray-500 italic text-sm">
                      Esta lista está vacía. Agrega animes desde el buscador o
                      la página principal.
                    </div>
                  )}
                </div>
